...
const envSchema = z.object({
  ...
  DB_NAME: z.string().min(1).optional().default('app_lautaro'),

  JWT_SECRET: z.string().min(16).optional().default('dev_secret_change_me_please'),
  JWT_EXPIRES_IN: z.string().min(1).optional().default('8h'),
});

const env = envSchema.parse(process.env);

if (env.NODE_ENV === 'production' && env.JWT_SECRET === 'dev_secret_change_me_please') {
